...
export default function StatsTablet({onBack,onNavigate}:{onBack:()=>void;onNavigate:(tab:Nav)=>void}){
 ...
 return <main className="tabletStats">
  ...
  <section className="tabletBody"><h1>THARTOS</h1><div className="titleRule"><span/>STATS<span/></div>
   ...
   <div className="lowerGrid"><section className="listPanel saves"><h2>Rettungswürfe</h2>{saves.map(([key,label])=><button key={key} onClick={()=>open(key,label)}><b>{key.toUpperCase()}</b><span>{label}</span><strong>{signed(key==='wis'||key==='cha'?mod(values[key])+2:mod(values[key]))}</strong><em>{key==='wis'||key==='cha'?'★':'○'}</em></button>)}<footer>★ geübt &nbsp;&nbsp; ○ nicht geübt</footer></section><section className="listPanel skillPanel"><h2>Fertigkeiten</h2><div>{skills.map(([name,,value,trained],index)=><button key={String(name)} onClick={()=>open(`skill${index}`,String(name))}><span>{name}</span><strong>{signed(values[`skill${index}`]??Number(value))}</strong><em>{trained?'★':'○'}</em></button>)}</div></section></div>
