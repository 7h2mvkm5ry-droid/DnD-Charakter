...
const defaults:Values={str:11,dex:14,con:16,int:13,wis:9,cha:15,ac:16,initiative:2,speed:10,hp:29,hpMax:29,tempHp:5,spellDc:12,spellAttack:4,proficiency:2,passive:9};
...
export default function StatsTablet({onBack,onNavigate}:{onBack:()=>void;onNavigate:(tab:Nav)=>void}){
 const[values,setValues]=useState<Values>(defaults);const[edit,setEdit]=useState<{key:string;label:string}|null>(null);const[draft,setDraft]=useState(0);
 ...
 const confirm=()=>{if(edit){save({...values,[edit.key]:draft});setEdit(null)}};
 ...
 return <main className="tabletStats">
  ...
  <section className="tabletBody"><h1>THARTOS</h1><div className="titleRule"><span/>STATS<span/></div>
   ...
   <div className="combatStrip"><SmallStat icon="✥" label="Rüstungsklasse" value={values.ac} onClick={()=>open('ac','Rüstungsklasse')}/><SmallStat icon="ϟ" label="Initiative" value={signed(values.initiative)} onClick={()=>open('initiative','Initiative')}/><SmallStat icon="♟" label="Bewegung" value={values.speed} onClick={()=>open('speed','Bewegung')}/>
    ...
    <button className={`tempPanel ${values.tempHp===0?'empty':''}`} onClick={reduceTemp}><span>⌛</span><label>Temporäre TP</label><strong>{values.tempHp}</strong><small>{values.tempHp>0?'Tippen: −1':'TP-Regler frei'}</small></button>
   </div>
