...
export default function StatsTablet({onBack,onNavigate}:{onBack:()=>void;onNavigate:(tab:Nav)=>void}){
 ...
 return <main className="tabletStats">
  ...
  <section className="tabletBody"><h1>THARTOS</h1><div className="titleRule"><span/>STATS<span/></div>
   <div className="abilityGrid">{abilityMeta.map(([key,label,,icon,tone])=><button className={`abilityCard ${tone}`} key={key} onClick={()=>open(key,label)}><span className="abilityIcon">{icon}</span><span className="abilityName">{label}</span><strong>{signed(mod(values[key]))}</strong><em>{values[key]}</em><i>›</i></button>)}</div>
   <div className="combatStrip"><SmallStat icon="✥" label="Rüstungsklasse" value={values.ac} onClick={()=>open('ac','Rüstungsklasse')}/><SmallStat icon="ϟ" label="Initiative" value={signed(values.initiative)} onClick={()=>open('initiative','Initiative')}/><SmallStat icon="♟" label="Bewegung" value={values.speed} onClick={()=>open('speed','Bewegung')}/>
    <button className="hpPanel" onClick={()=>open('hp','Aktuelle Trefferpunkte')} aria-label="Aktuelle Trefferpunkte ändern"><span className="heart">♥</span><span className="hpContent"><span className="hpLabel">Trefferpunkte</span><span className="hpValue"><strong>{values.hp}</strong> <span>/ {values.hpMax}</span></span><span className="hpTrack"><i style={{width:`${hpPercent}%`}}/></span></span></button>
    <button className={`tempPanel ${values.tempHp===0?'empty':''}`} onPointerDown={startTempPress} onPointerUp={endTempPress} onPointerLeave={()=>tempTimer.current&&clearTimeout(tempTimer.current)} onContextMenu={e=>e.preventDefault()}><span>⌛</span><label>Temporäre Trefferpunkte</label><strong>{values.tempHp}</strong><small>{values.tempHp>0?'Tippen −1 · Halten ändern':'Halten zum Ändern'}</small></button>
   </div>
...
function SmallStat({icon,label,value,onClick,wide=false}:{icon:string;label:string;value:string|number;onClick:()=>void;wide?:boolean}){return <button className={`smallStat ${wide?'wide':''}`} onClick={onClick}><span>{icon}</span><label className={label==='Rüstungsklasse'?'noWrap':''}>{label}</label><strong>{value}</strong><i>›</i></button>}
